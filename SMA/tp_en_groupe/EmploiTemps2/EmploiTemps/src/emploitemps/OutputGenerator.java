...
public class OutputGenerator {
	...
	String[] wDay = { "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi" };
	String[] wHour = { "Heure 1", "Heure 2", "Heure 3", "Heure 4", "Heure 5",
	...
	public void toPDF(String fname, Matrix2D<String> tt) {
		...
		try {
			...
			document.open();
			Paragraph title1 = new Paragraph(fname + " Emploi de temps", FontFactory
			...
			MSGBOX("Création échouée", e2.getMessage());
		}
	}
	...
	public void toHTML(String fname, Matrix2D<String> tt) {
		...
		try {
			...
			String html = "<html>\n";
			html += "<title>" + fname + " Emploi de temps</title>\n";
			html += "<body><h1>" + fname + " Emploi de temps</h1>\n";
	...
	public void AllHTML(JFrame f) {
		...
		if (str != "") {
			TTGSProgressBar tpb = new TTGSProgressBar(str, "Format HTML", HTML,f);
		}
	}
	...
	public void AllPDF(JFrame f) {
		String str = OnlyFormat();
		if (str != "") {
			TTGSProgressBar tpb = new TTGSProgressBar(str, "Format PDF", PDF, f);
		}
	}
	...
	class TTGSProgressBar {
		...
		public TTGSProgressBar(String str1, String title, int format1,
			...
			if (str == "") {
				title = "Aucun emploi de temps à exporter " + title;
				button.setEnabled(false);
			} else
				title = str + " emploi de temps à exporter sous " + title;
			...
			timer = new Timer(interval, new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					if (i == 20) {
						...
						label.setText(str1);
						if (format == PDF)
							MSGBOX("Emploi du temps",str	+ " Emploi de temps (au format .PDF ) Génération réussie!");
						else
							MSGBOX("Emploi du temps",str	+ " Emploi de temps (au format .HTML) Génération réussie!");
