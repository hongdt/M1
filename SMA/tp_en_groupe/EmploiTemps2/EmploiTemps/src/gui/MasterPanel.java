...
public class MasterPanel extends TTGSModule {
	...
	public JPanel getTeacharContent(String title) {
		...
		editBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (editBtn.getText().equalsIgnoreCase("Modifier Enseignant")) {
	...
	public JPanel getSubjectContent(String title) {
		...
		addBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(addBtn.getText().compareToIgnoreCase("Ajouter Cours")==0){
		...
		editBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (editBtn.getText().equalsIgnoreCase("Modifier Cours")) {
					if (tableView.getSelectedRow() != -1) {
						EditRow(tableView.getSelectedRow());
					...
					if (tableView.getSelectedRow() != -1) {
						if (SaveRow(tableView.getSelectedRow())) {
							RefreshTable(P0,DataSet.SUBJECT, subjnames);
							editBtn.setText("Modifier Cours");
							addBtn.setEnabled(true);
							JOptionPane.showMessageDialog(null,"Les lignes ont été correctement enregistrée[" + tableView.getSelectedRow() + "] de " + tableView.getName(),"Enregistrer", JOptionPane.INFORMATION_MESSAGE);
						}
					} else
	...
	public JPanel getRoomContent(String title) {
		...
		jcbATag.setModel(new DefaultComboBoxModel(new String[] { "Sélectionner Salle", "SALLE", "LABO"}));
