...
import metier.ActivitySet;
import metier.AssigndSubjects;
import metier.DataSet;
import metier.SubjectSet;
import emploitemps.Matrix2D;
import emploitemps.Matrix3D;
import emploitemps.Random;
...
public class TTGSGenerator {
	...
	public void MainAlgo(JFrame f) {
		initMatrices();
		TTGSProgressBar tpb = new TTGSProgressBar(
				ds.getActivityList().size() / 2, f);
	...
	class TTGSProgressBar {
		...
		public TTGSProgressBar(int work1, JFrame baseFrame) {
			...
			if (!isActivitiesGenerated()) {
				title = "Aucune activité crée";
				button.setEnabled(false);
			} else
				title = "Génération de l'emploi de temps !";

			pb = new JProgressBar(0, work);
			pb.setValue(0);
			pb.setStringPainted(true);

			label = new JLabel("Génération de l'emploi de temps en cours :");
